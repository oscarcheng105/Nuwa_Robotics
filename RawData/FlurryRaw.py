...
def checkEndTime(endD):
    while(True):
        try:
            ...
            endD = input("ENDTIME INPUT INVALID, EndTime cannot be later than DefaultTime (3h before current time), Please reinput (yyyy-mm-dd hh:mm:ss): ")
...
def checkStartTime(startD,endD=None):
    while(True):
        try:
            ...
            startD = input("STARTTIME INPUT INVALID, StartTime cannot be later than EndTime, Please reinput (yyyy-mm-dd hh:mm:ss): ")
